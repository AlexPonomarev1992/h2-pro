  ...
  useEffect(() => {
    ...
    try {
      ...
      cityLocations.forEach((city, index) => {
        ...
        el.style.cursor = 'pointer'; el.style.opacity = '0';
        el.style.transition = 'opacity 0.5s ease-in-out';
        
        const innerEl = document.createElement('div');
        innerEl.style.width = '100%'; innerEl.style.height = '100%';
        innerEl.style.backgroundImage = 'url(/assets/marker.png)';
        innerEl.style.backgroundSize = 'contain'; innerEl.style.mixBlendMode = 'screen';
        el.appendChild(innerEl);

        const isThisCitySubmitted = submittedCityName === city.name;
        const popup = new mapboxgl.Popup({ offset: 25 }).setHTML(`
          <div style="padding: 12px; min-width: 200px; background: #0B121B; color: #fff; border-radius: 8px;">
            <div style="font-weight: 700; color: #00f0ff; margin-bottom: 4px;">${city.name}</div>
            ...
            <button onclick="${isThisCitySubmitted ? '' : `window.openBookingForm('${city.name}')`}"
              style="width: 100%; padding: 10px; background: ${isThisCitySubmitted ? 'transparent' : 'linear-gradient(90deg, #00f0ff, #0072ff)'}; color: ${isThisCitySubmitted ? '#00f0ff' : '#000'}; border: ${isThisCitySubmitted ? '1px solid #00f0ff' : 'none'}; border-radius: 6px; font-weight: bold;"
            >
    ...
    return () => {
      if (map.current) { map.current.remove(); map.current = null; markers.current = []; }
    };
  }, [viewMode, submittedCityName]);

  const filteredCities = cityLocations.filter(city => selectedRegion === "Все" || city.region === selectedRegion);

  return (
    <>
      <div className="space-y-4">
        {/* Переключатель Карта/Список */}
        <div className="flex bg-[#0B121B] p-1 rounded-lg border border-border w-fit">
          <button 
            onClick={() => setViewMode('list')}
            className={`px-6 py-2 rounded-md transition-all ${viewMode === 'list' ? 'bg-[#00f0ff] text-black font-bold' : 'text-gray-400 hover:text-white'}`}
          >
            Список городов
          </button>
          <button 
            onClick={() => setViewMode('map')}
            className={`px-6 py-2 rounded-md transition-all ${viewMode === 'map' ? 'bg-[#00f0ff] text-black font-bold' : 'text-gray-400 hover:text-white'}`}
          >
            Карта
          </button>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex flex-wrap gap-2">
            {regions.map((region) => (
              <GlowButton key={region} variant={selectedRegion === region ? "primary" : "outline"} size="sm" onClick={() => setSelectedRegion(region)}>
                {region}
              </GlowButton>
            ))}
          </div>
        ...
        <div className="relative w-full min-h-[500px]">
          {viewMode === 'map' ? (
            <div ref={mapContainer} className="w-full h-[500px] rounded-lg border border-border shadow-lg" style={{ background: '#0B121B' }} />
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredCities.map((city) => {
                const isThisCitySubmitted = submittedCityName === city.name;
                return (
                  <div key={city.name} className="bg-[#0B121B] border border-border p-5 rounded-xl hover:border-[#00f0ff]/50 transition-colors group">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h4 className="text-lg font-bold text-white group-hover:text-[#00f0ff] transition-colors">{city.name}</h4>
                        <div className="flex items-center gap-2 text-sm text-gray-400 mt-1">
                          <MapPin className="w-4 h-4 text-[#00f0ff]" /> {city.address}
                        </div>
                      </div>
                      <span className="text-[10px] uppercase tracking-wider px-2 py-1 bg-white/5 rounded border border-white/10 text-gray-500">
                        {city.serviceType === 'truck' ? 'Грузовой' : city.serviceType === 'passenger' ? 'Легковой' : 'Универсальный'}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-4 mt-6 pt-4 border-t border-white/5">
                      ...
                      {isThisCitySubmitted ? (
                        <div className="text-[#00f0ff] font-bold text-sm border border-[#00f0ff] px-4 py-2 rounded-lg bg-[#00f0ff]/5">
                          Запись создана
                        </div>
                      ) : (
                        <GlowButton variant="primary" size="sm" onClick={() => handleOpenForm(city.name)}>
                          Записаться
                        </GlowButton>
                      )}
                    </div>
            ...
            <form className="space-y-4" onSubmit={async (e) => {
              ...
              try {
                const contactRes = await fetch('https://h2pro.bitrix24.ru/rest/1/xmv4aig8i7ug15lw/crm.contact.add.json', {
                  ...
                  body: JSON.stringify({
                    fields: { NAME: userName, PHONE: [{ "VALUE": userPhone, "VALUE_TYPE": "WORK" }] }
                  })
                });
                ...
                if (dealData.result) { setSubmittedCityName(selectedCityForBooking.name); setShowBookingForm(false); }
              } catch (error) { alert("Ошибка при отправке."); }
            }}>
              <input name="userName" placeholder="Ваше имя" required className="w-full bg-[#0F1621] border border-border p-2 rounded text-white outline-none" />
              <input name="userPhone" type="tel" placeholder="+7 (999) 000-00-00" required className="w-full bg-[#0F1621] border border-border p-2 rounded text-white outline-none" />
